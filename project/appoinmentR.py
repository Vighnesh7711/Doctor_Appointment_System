...
def fetch_time_slots(doctor_id, selected_date):
    ...
    if not db:
        return []
    cursor = db.cursor()
    cursor.execute("SELECT time_slot FROM appointmentsEmer1 WHERE doctor_id = %s AND date = %s", (doctor_id, selected_date))
...
def book_appointment(time_slot):
    ...
    cursor = db.cursor()
    cursor.execute(
        "INSERT INTO appointmentsEmer1 (doctor_id, date, time_slot, patient_name, description,contact_number) VALUES (%s, %s, %s, %s, %s, %s)",
